...
def show_transactions_page(user_id):
    ...
    if not transactions_df.empty:
        ...
        with st.expander("Delete a Transaction"):
            # Updated format_func to handle numpy.datetime64 objects
            transaction_to_delete = st.selectbox(
                "Select transaction to delete", 
                transactions_df['id'].tolist(),
                format_func=lambda x: f"{pd.Timestamp(transactions_df[transactions_df['id'] == x]['date'].values[0]).strftime('%Y-%m-%d')} - {transactions_df[transactions_df['id'] == x]['category'].values[0]} - ${transactions_df[transactions_df['id'] == x]['amount'].values[0]:.2f}"
            )
